...
public class AzimuthTestEC {
    ...
    @Test
    public void toOctantStringCorrectOnLimits(){
        String n = "north", e = "east", s = "south", w = "west";
        ArrayList<String> expected = new ArrayList<>();
        expected.add(n+e);expected.add(n+e);
        expected.add(s+e);expected.add(s+e);
        expected.add(s+w);expected.add(s+w);
        expected.add(n+w);expected.add(n+w);
        for(int aDeg = 0; aDeg < 8; ++aDeg){
            double aRad = toRadians((aDeg*45+22.5));
            String os = toOctantString(aRad, n, e, s, w);
            assertEquals(expected.get(aDeg), os);
        }
    }
